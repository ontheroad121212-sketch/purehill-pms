...
with st.sidebar:
    ...
    st.caption("v14.0: 시뮬레이션 및 세그먼트 심층 대조 풀버전")
...
if not prod_data.empty:
    ...
    def render_booking_dashboard(curr_df, prev_df, title_label, current_label, prev_label):
        ...
        # 4. 거래처별 심층 분석 (LOS, 리드타임 포함 그래프 4종)
        st.write("---")
    ...
    # 🚀 [v14.0 핵심] 미래 OTB 탭 - 4개월 달성 현황 및 시뮬레이션
    with tab_f:
        if not otb_data.empty:
            ...
            if not month_otb.empty:
                st.error(f"🚨 {cur_month}월 버짓 달성 시뮬레이션")
                last_day = calendar.monthrange(latest_booking_date.year, cur_month)[1]
                days_left = last_day - latest_booking_date.day
                if days_left > 0:
                    c_rev, c_rn = month_otb['합계_매출'].sum(), month_otb['합계_객실'].sum()
                    s_rev, s_rn = m_bud['rev_won'] - c_rev, m_bud['rn'] - c_rn
            ...
            if st.button("🤖 AI 미래 전략 리포트"):
                if api_key: st.info(get_ai_insight(api_key, "향후 4개월 버짓 대비 OTB 상황을 분석하여 수익 극대화 전략을 제안해줘."))
else:
