...
# 1. 화면 설정
st.set_page_config(page_title="엠버퓨어힐 통합 관제 v14.0", layout="wide")
...
with st.sidebar:
    ...
    st.caption("v14.0: 시뮬레이션 및 전 세그먼트 심층 대조 완결본")
...
if not prod_data.empty:
    ...
    def render_booking_dashboard(curr_df, prev_df, title_label, current_label, prev_label):
        ...
        bc2.metric("FIT 조식 비중", f"{get_bf_ratio(f_curr):.1f}%")
        bc3.metric("Group 조식 비중", f"{get_bf_ratio(g_curr):.1f}%")
        ...
        # 🚀 [FIT / Group 세그먼트별 직관 대조 - 전체와 동일 로직]
        st.write("---")
        ...
        fc3.metric("FIT RN", f"{ft_rn:,.0f} RN", delta=f"{int(ft_rn - fp_rn):+d} RN (전기: {fp_rn:,.0f})")
        fc4.metric("FIT ADR (Net)", f"{ft_adr:,.0f}원", delta=f"{get_delta_pct(ft_adr, fp_adr)} (전기: {fp_adr:,.0f})")
        ...
        gc3.metric("그룹 RN", f"{gt_rn:,.0f} RN", delta=f"{int(gt_rn - gp_rn):+d} RN (전기: {gp_rn:,.0f})")
        gc4.metric("그룹 ADR (Net)", f"{gt_adr:,.0f}원", delta=f"{get_delta_pct(gt_adr, gp_adr)} (전기: {gp_adr:,.0f})")
        ...
        with g_c2: st.plotly_chart(px.bar(acc_stats.sort_values('Net_ADR').tail(10), x='Net_ADR', y='account', orientation='h', title="거래처별 순수 객실 ADR", text_auto=',.0f', color_continuous_scale='Greens', color='Net_ADR'), use_container_width=True)
        g_c3, g_c4 = st.columns(2)
        with g_c3: st.plotly_chart(px.bar(acc_stats.sort_values('los').tail(10), x='los', y='account', orientation='h', title="거래처별 평균 LOS (박수)", text_auto='.1f', color_continuous_scale='Purples', color='los'), use_container_width=True)
        with g_c4: st.plotly_chart(px.bar(acc_stats.sort_values('lead_time').tail(10), x='lead_time', y='account', orientation='h', title="거래처별 평균 리드타임 (일)", text_auto='.1f', color_continuous_scale='Oranges', color='lead_time'), use_container_width=True)
        ...
        if st.button(f"🤖 AI [{title_label}] 실적 분석 리포트", key=f"ai_{title_label}"):
            if api_key: st.info(get_ai_insight(api_key, f"매출:{t_tot:,.0f}, RN:{t_rn}, ADR:{t_adr:,.0f} 실적을 분석해줘."))
    ...
    with tab_f:
        if not otb_data.empty:
            ...
            if not month_otb.empty:
                ...
                if days_left > 0:
                    c_rev = month_otb['합계_매출'].sum()
                    c_rn = month_otb['합계_객실'].sum()
                    s_rev = m_bud['rev_won'] - c_rev
                    s_rn = m_bud['rn'] - c_rn
                    req_rn = max(0, s_rn / days_left)
                    req_adr = (s_rev / s_rn) if s_rn > 0 else 0
                    
                    sc1, sc2, sc3 = st.columns(3)
                    sc1.metric("남은 기간", f"{days_left}일")
                    sc2.metric("일일 필요 판매량", f"{req_rn:.1f} RN/일")
                    sc3.metric("필요 평균 단가(Net)", f"{req_adr:,.0f}원")
